...
SOFA_DECL_CLASS(MeshGmshLoaderCNVSS)

int MeshGmshLoaderCNVSSClass = core::RegisterObject("Specific mesh loader for Gmsh file format.")
        .add< MeshGmshLoaderCNVSS >()
        ;

bool MeshGmshLoaderCNVSS::load()
{
  std::cout << "MeshGmshLoaderCNVSS::load()" << std::endl;
    ...
    {
        serr << "Error: MeshGmshLoaderCNVSS: File '" << m_filename << "' finally appears not to be a Gmsh file." << sendl;
        file.close();
        return false;
    }
...
void MeshGmshLoaderCNVSS::addInGroup(helper::vector< sofa::core::loader::PrimitiveGroup>& group,int tag,int /*eid*/) {
...
void MeshGmshLoaderCNVSS::normalizeGroup(helper::vector< sofa::core::loader::PrimitiveGroup>& group) {
    int start = 0;
    for (unsigned i=0;i<group.size();i++) {
        group[i].p0 = start;
        start += group[i].nbp;
...
bool MeshGmshLoaderCNVSS::readGmsh(std::ifstream &file, const unsigned int gmshFormat)
{
    ...
    {
        serr << "Error: MeshGmshLoaderCNVSS: '$ENDNOD' or '$EndNodes' expected, found '" << cmd << "'" << sendl;
        file.close();
        return false;
    }
    ...
    {
        serr << "Error: MeshGmshLoaderCNVSS: '$ELM' or '$Elements' expected, found '" << cmd << "'" << sendl;
        file.close();
        return false;
    }
    ...
    for (unsigned int i=0; i<nelems; ++i) // for each elem
    {
        ...
        else /*if (gmshFormat == 2)*/
        {
            ...
            case 11: // Quadratic Tetrahedron
                ...
                break;
            default:
                serr << "Error: MeshGmshLoaderCNVSS: Elements of type 1, 2, 3, 4, 5, or 6 expected. Element of type " << etype << " found." << sendl;
                nnodes = 0;
            }
        }
    ...
    {
        serr << "Error: MeshGmshLoaderCNVSS: '$ENDELM' or '$EndElements' expected, found '" << cmd << "'" << sendl;
        file.close();
        return false;
    }
