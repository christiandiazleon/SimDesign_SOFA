...
******************************************************************************/
#ifndef SOFA_COMPONENT_FORCEFIELD_TETRAHEDRALCOROTATIONALFEMFORCEFIELDCNVSS_INL
#define SOFA_COMPONENT_FORCEFIELD_TETRAHEDRALCOROTATIONALFEMFORCEFIELDCNVSS_INL
...
void TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::TetrahedronHandler::applyCreateFunction(unsigned int tetrahedronIndex,
...
template< class DataTypes>
TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::TetrahedralCorotationalFEMForceFieldCNVSS()
...
template <class DataTypes>
void TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::init()
{

    std::cout << "TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::init()" << std::endl;

    //PointNet* pntTemp = new PointNet(1.0f, 2.0f, 3.0f);
    //std::cout << pntTemp->getX() << " " << pntTemp->getY() << " " << pntTemp->getZ() << std::endl;

    //simulation::Node *tempNode =(simulation::Node *) this->getContext();
    ...
    {
        serr << "ERROR(TetrahedralCorotationalFEMForceFieldCNVSS): object must have a Tetrahedral Set Topology."<<sendl;
        return;
    }
    ...
    reinit(); // compute per-element stiffness matrices and other precomputed values
      ...
      /*int oldIndex = _topology->getIndexCurrentMesh();
    	...
    	for(unsigned int i=0; i<_topology->getNbOfMeshes();i++)
    	{
    		...
    		helper::vector<EdgeRestInformation>& edgeInf = *(edgeInfoMeshes[i].beginEdit());

    		/// prepare to store info in the edge array
    		edgeInf.resize(_topology->getNbEdges());

    		if (_initialPointsMeshes[i].size() == 0)
    		{
    			// get restPosition
    			const VecCoord& p = *mObject->getX0();
    			_initialPointsMeshes[i]=p;
    		}

    		int j;
    		// set edge tensor to 0
    		for (j=0;j<_topology->getNbEdges();++j) {
    			TetrahedralTMEdgeCreationFunction(j, (void*) this, edgeInf[j],
    				_topology->getEdge(j),  (const std::vector< unsigned int > )0,
    				(const std::vector< double >)0);
    		}

    		// create edge tensor by calling the tetrahedron creation function
    		std::vector<unsigned int> tetrahedronAdded;
    		for (j=0;j<_topology->getNbTetrahedra();++j)
    			tetrahedronAdded.push_back(j);

    		TetrahedralTMTetrahedronCreationFunction(tetrahedronAdded,(void*) this,
    			edgeInf);

    		edgeInfoMeshes[i].setCreateFunction(TetrahedralTMEdgeCreationFunction);
    		edgeInfoMeshes[i].setCreateTetrahedronFunction(TetrahedralTMTetrahedronCreationFunction);
    		//edgeInfoMeshes[i].setDestroyTetrahedronFunction(TetrahedralTMTetrahedronDestructionFunction);
    		edgeInfoMeshes[i].setCreateParameter( (void *) this );
    		edgeInfoMeshes[i].setDestroyParameter( (void *) this );

    		edgeInfoMeshes[i].endEdit();
...
template <class DataTypes>
void TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::reinit()
{
...
template<class DataTypes>
void TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::computeStrainDisplacement( StrainDisplacementTransposed &J, Coord a, Coord b, Coord c, Coord d )
{
...
template<class DataTypes>
typename TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::Real TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::peudo_determinant_for_coef ( const defaulttype::Mat<2, 3, Real>&  M )
{
    return  M[0][1]*M[1][2] - M[1][1]*M[0][2] -  M[0][0]*M[1][2] + M[1][0]*M[0][2] + M[0][0]*M[1][1] - M[1][0]*M[0][1];
}

template<class DataTypes>
void TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::computeStiffnessMatrix( StiffnessMatrix& S,StiffnessMatrix& SR,const MaterialStiffness &K, const StrainDisplacementTransposed &J, const Transformation& Rot )
{
...
template<class DataTypes>
void TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::computeMaterialStiffness(MaterialStiffness& materialMatrix, Index&a, Index&b, Index&c, Index&d, SReal localStiffnessFactor)
{
...
//    /// @TODO: in TetrahedronFEMForceField, the stiffness matrix is divided by 6 compared to the code in TetrahedralCorotationalFEMForceFieldCNVSS. Check which is the correct one...
...
template<class DataTypes>
inline void TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::computeForce( Displacement &F, const Displacement &Depl, const MaterialStiffness &K, const StrainDisplacementTransposed &J )
{
...
template<class DataTypes>
inline void TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::computeForce( Displacement &F, const Displacement &Depl, const MaterialStiffness &K, const StrainDisplacementTransposed &J, SReal fact )
{
...
template<class DataTypes>
void TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::initSmall(int i, Index&a, Index&b, Index&c, Index&d)
{
    const VecCoord X0=this->mstate->read(core::ConstVecCoordId::restPosition())->getValue();
...
template<class DataTypes>
void TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::accumulateForceSmall( Vector& f, const Vector & p,Index elementIndex )
{
    ...
    /*        serr<<"TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::accumulateForceSmall, displacement"<<D<<sendl;
            serr<<"TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::accumulateForceSmall, strainDisplacementTransposed"<<tetrahedronInfo[elementIndex].strainDisplacementTransposedMatrix<<sendl;
            serr<<"TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::accumulateForceSmall, material"<<tetrahedronInfo[elementIndex].materialMatrix<<sendl;*/
    ...
    if(!_assembling.getValue())
    {
        ...
        //serr<<"TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::accumulateForceSmall, force"<<F<<sendl;
    }
    else
    {
        Transformation Rot;
...
template<class DataTypes>
void TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::applyStiffnessSmall( Vector& f, const Vector& x, int i, Index a, Index b, Index c, Index d, SReal fact )
{
    Displacement X;
...
template<class DataTypes>
inline void TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::computeRotationLarge( Transformation &r, const Vector &p, const Index &a, const Index &b, const Index &c)
{
...
template <class DataTypes>
inline void TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::getElementRotation(Transformation& R, unsigned int elementIdx)
{
...
template <class DataTypes>
inline void TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::getRotation(Transformation& R, unsigned int nodeIdx)
{
    const helper::vector<typename TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::TetrahedronInformation>& tetraInf = tetrahedronInfo.getValue();
...
template <class DataTypes>
inline void TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::getElementStiffnessMatrix(Real* stiffness, unsigned int elementIndex)
{
    const helper::vector<typename TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::TetrahedronInformation>& tetraInf = tetrahedronInfo.getValue();
    Transformation Rot;
    StiffnessMatrix JKJt,tmp;
...
template <class DataTypes>
inline void TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::getElementStiffnessMatrix(Real* stiffness, core::topology::BaseMeshTopology::Tetra& te)
{
    const VecCoord X0=this->mstate->read(core::ConstVecCoordId::restPosition())->getValue();
...
template<class DataTypes>
void TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::initPolar(int i, Index& a, Index&b, Index&c, Index&d)
{
    const VecCoord X0=this->mstate->read(core::ConstVecCoordId::restPosition())->getValue();
...
template<class DataTypes>
void TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::accumulateForcePolar( Vector& f, const Vector & p, Index elementIndex )
{
    const core::topology::BaseMeshTopology::Tetrahedron t=_topology->getTetrahedron(elementIndex);
    ...
    {
        serr << "TODO(TetrahedralCorotationalFEMForceFieldCNVSS): support for assembling system matrix when using polar method."<<sendl;
    }
...
template<class DataTypes>
void TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::applyStiffnessPolar( Vector& f, const Vector& x, int i, Index a, Index b, Index c, Index d, SReal fact )
{
    helper::vector<typename TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::TetrahedronInformation>& tetrahedronInf = *(tetrahedronInfo.beginEdit());
...
template<class DataTypes>
void TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::addKToMatrix(sofa::defaulttype::BaseMatrix *mat, SReal k, unsigned int &offset)
{
...
template<class DataTypes>
void TetrahedralCorotationalFEMForceFieldCNVSS<DataTypes>::printStiffnessMatrix(int idTetra)
{
...
#endif // SOFA_COMPONENT_FORCEFIELD_TETRAHEDRALCOROTATIONALFEMFORCEFIELDCNVSS_INL
